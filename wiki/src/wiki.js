...
function randomSearch(randomPromise, cb) {
    //return new Promise((resolve, reject) => {
        wiki().search(randomPromise, 100)
            .then((data) => {
                // console.log(data.results);
                for (let i = 0; i < data.results.length; i++) {
                    if (data.results[i].split(" ")[0] === randomPromise.capitalize()
                ...
                if (newWordArray.length > 1) {
                    ...
                    let secondSearchQeury = newWordArray[Math.floor(Math.random() * newWordArray.length)]
                    concatArray.push(secondSearchQeury);
                    let newSecondSearchQuery = secondSearchQeury.split(" ").pop().replace(/[{()}]/g, "");
                    //let newWordQuery = newSecondSearchQuery;
                    newWordArray = [];
                    if (concatArray.length >= 25) {
                        console.log('here');
